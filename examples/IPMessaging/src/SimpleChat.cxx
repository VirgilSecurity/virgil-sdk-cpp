...
void vipm::SimpleChat::onMessageRecived(const std::string& sender, const std::string& message) {
    ...
    auto foundCards = servicesHub_.card().searchGlobal(sender, vsdk::dto::IdentityType::Email);
    if (foundCards.empty()) {
        return;
    }
...
vipm::ChatMember vipm::SimpleChat::autorize(const std::string& emailAddress) {
    std::vector<vsdk::models::CardModel> foundCards =
        servicesHub_.card().searchGlobal(emailAddress, vsdk::dto::IdentityType::Email);
    ...
    vsdk::dto::ValidatedIdentity validatedIdentity = identityConfirm(emailAddress);
...
vipm::ChatMember vipm::SimpleChat::registerUser(const std::string& emailAddress) {
...
MapCardIdPublicKey vipm::SimpleChat::getChannelRecipients() {
    ...
    for (const auto& channelMember : channelMembers) {
        auto foundCards = servicesHub_.card().searchGlobal(channelMember, vsdk::dto::IdentityType::Email);
...
vsdk::dto::ValidatedIdentity vipm::SimpleChat::identityConfirm(const std::string& email) {
    ...
    return servicesHub_.identity().confirm(actionId, confirmationCode);
}
